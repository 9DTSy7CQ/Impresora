...
public class RegistroImpresion {
    ...
    private static final String FORMATO_OK =
    ...
    private RegistroImpresion() {}

    public static RegistroImpresion getInstancia() {
        return INSTANCIA;
    }

    public synchronized void registrarOK(String tipo, int hojas, double coste, int bn, int color) {
        escribir(String.format(FORMATO_OK, LocalDateTime.now(), tipo, hojas, coste, bn, color));
    }

    public synchronized void registrarKO(String tipo, int hojas, int bn, int color) {
